...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (contactForm) {
        ...
        if (isCompassAttached) {
            ...
            // Populate hidden input for email delivery from serialized report
            populateDesignCompassField();
        } else {
        ...
        // Helper to format The Design Compass report as structured plaintext for Web3Forms email notification
        function formatCompassReportText(parsed) {
            if (!parsed) return '';
            let report = `=== THE DESIGN COMPASS: SPATIAL VISION REPORT ===\n\n`;
            report += `ARCHITECTURAL PERSONA: ${parsed.persona || 'Custom'}\n`;
            if (parsed.typology) report += `TYPOLOGY: ${parsed.typology}\n`;
            if (parsed.strapline) report += `STRAPLINE: ${parsed.strapline}\n`;
            report += `\n`;

            if (parsed.keyPrinciples && parsed.keyPrinciples.length) {
                report += `KEY PRINCIPLES:\n`;
                parsed.keyPrinciples.forEach((kp, idx) => {
                    report += `${idx + 1}. ${kp.title}: ${kp.text}\n`;
                });
                report += `\n`;
            }

            if (parsed.selections && parsed.selections.length) {
                report += `12 CALIBRATED SELECTIONS:\n`;
                parsed.selections.forEach(s => {
                    report += `• Chapter ${String(s.chapter).padStart(2, '0')} [${s.category}]: Option ${s.option} — ${s.title} (${s.tag || ''})\n`;
                });
                report += `\n`;
            }
        ...
        // Populate hidden input field for Web3Forms email notification
        function populateDesignCompassField() {
            ...
            try {
                ...
                console.warn("Error reading/formatting Design Compass report for submission:", err);
            }
        }
        ...
        contactForm.addEventListener('submit', async (e) => {
            ...
            // Ensure hidden Design Compass report field is populated from sessionStorage if available
            populateDesignCompassField();
